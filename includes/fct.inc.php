 ...
 * @return null
 */
function connecterV($idVisiteur, $nom, $prenom, $statut)
{
...
function connecterC($idComptable, $nom, $prenom, $statut)
{
...
function get12DernierMois($mois)
{
    ...
    $listeMois = array();
    for ($k=1; $k<=12;$k++){
        $listeMois[$k] = ($numMois-1).'/'.$numAnnee;
        if ($numMois == 1){
            $listeMois[$k]= ($numMois = '12').'/'.($numAnnee - 1);
        }
        else{
            $numMois = ($numMois - 1);
        }
    ...
    for ($k=1; $k<=12;$k++){
        if (strlen($listeMois[$k]) == 1) {
        $listeMois = '0' . $listeMois;
    }   
    }
    return $listeMois;
}
